...
void get_reference_with_rb_bit(int fd, t_reference *reference) {
  ...
  for (int i = 0; i < REFERENCE_SIZE; i++)
    scanf("%d %c\nc", &reference[i].page, &reference[i].rw_bit);
...
// 프레임 배열을 출력하는 함수
void print_frame_array(int page, int *frame, int n_frames, char *result,
                       int fd) {
  // 참조값 출력
  printf("%10d", page);
  // 참조값 파일에 저장
...
// 프레임 연결 리스트를 출력하는 함수
void print_frame_list(int page, t_frame *head, int n_frames, char *result,
                      int fd) {
  t_frame *tmp = head;
  int cnt = 0;
...
void print_frame_with_r_bit(int page, t_frame_with_r_bit *frame, int n_frames,
                            char *result, int fd) {
  // 참조값 출력
  printf("%10d", page);
  // 참조값 파일에 저장
...
void print_frame_with_rw_bit(t_reference reference, t_frame_with_rw_bit *frame,
                             int n_frames, char *result, int fd) {
  // 참조값 출력
...
void print_result(char *algorithm_type, int page_fault, int fd) {
  float page_fault_rate =
      page_fault > 0 ? page_fault / (float)REFERENCE_SIZE * 100 : 0;
